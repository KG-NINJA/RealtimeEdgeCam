...
【プライバシーおよびデータ通信に関する表記】 本システムは高度なプライバシー保護と低遅延制御のため、
**「エッジ計算（端末内処理）」**を基本設計としています。

映像データについて: カメラ映像は、ご利用のブラウザ内（ローカル環境）でOpenCVおよびTensorFlow.jsを用いて即座に解析されます。
映像データそのものが外部サーバーへ送信、または保存されることは一切ありません。
AI判断（Gemma）への送信データ: Hugging Face上の推論サーバーへ送られるのは、
ローカルで計算された**「物体までの物理的な距離数値」および「機体の速度状態」などの匿名文字情報のみ**です。視覚情報は一切含まれません。
